...
public class DebtProfileActivity extends AppCompatActivity implements View.OnClickListener, EasyPermissions.PermissionCallbacks {

    private DebtProfileFragmentBinding binding;
    ...
    private void initLayout() {
        ...
        if (HomeFragment.mData.getTradelines().size() > 0 && HomeFragment.mData.getTradelines() != null) {
                ...
                if (Item.getAccount_Status().equalsIgnoreCase("active")) {
                    try {
                        ...
                        if (Item.getCurrent_Balance() != null) {
                            if (!Item.getCurrent_Balance().equalsIgnoreCase("")) {
                                total_balance = total_balance + Integer.parseInt(Item.getCurrent_Balance());
                            }
                        }
                        if (Item.getHighest_Credit_or_Original_Loan_Amount() != null) {
                            if (!Item.getHighest_Credit_or_Original_Loan_Amount().equalsIgnoreCase("")) {
            ...
            DebtTradeListAdapter activeAdapter = new DebtTradeListAdapter(mActiveData, dataEMI, DebtProfileActivity.this);
            binding.recyclerView.setHasFixedSize(true);
            binding.recyclerView.setNestedScrollingEnabled(false);
            binding.recyclerView.setLayoutManager(new LinearLayoutManager(DebtProfileActivity.this));
            binding.recyclerView.setAdapter(activeAdapter);

            DebtTradeListAdapter adapter = new DebtTradeListAdapter(mCloseData, closeDataEMI, DebtProfileActivity.this);
            binding.recyclerViewClose.setHasFixedSize(true);
            binding.recyclerViewClose.setNestedScrollingEnabled(false);
            binding.recyclerViewClose.setLayoutManager(new LinearLayoutManager(DebtProfileActivity.this));
            binding.recyclerViewClose.setAdapter(adapter);


            if (overdueList != null && overdueList.size() > 0) {
                OverdueTradeListAdapter overdueAdapter = new OverdueTradeListAdapter(overdueList, DebtProfileActivity.this);
                binding.recyclerViewOverdue.setHasFixedSize(true);
                binding.recyclerViewOverdue.setNestedScrollingEnabled(false);
                binding.recyclerViewOverdue.setLayoutManager(new LinearLayoutManager(DebtProfileActivity.this));
    ...
    private boolean checkPermission() {
        int result = ContextCompat.checkSelfPermission(DebtProfileActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE);
    ...
    private void requestPermission() {
        if (SDK_INT >= Build.VERSION_CODES.R) {
            ...
            if (ActivityCompat.shouldShowRequestPermissionRationale(DebtProfileActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                Toast.makeText(DebtProfileActivity.this, "Write External Storage permission allows us to save files. Please allow this permission in App Settings.", Toast.LENGTH_LONG).show();
            } else {
                ActivityCompat.requestPermissions(DebtProfileActivity.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, PERMISSION_REQUEST_CODE);
            }
        }
...
//            Toast.makeText(getContext(), e.getMessage(), Toast.LENGTH_SHORT).show();
            ...
            Logger.errorLogger("loanSummaryCrash", ex.getMessage());
        }
    }
    ...
    private void openXLS(final String path) {
        ...
        if (SDK_INT >= Build.VERSION_CODES.M) {
            uri = FileProvider.getUriForFile(DebtProfileActivity.this, getPackageName() + ".provider", file);
        } else {
        ...
        try {
            ...
            Toast.makeText(DebtProfileActivity.this, "You don't have application to open this Excel file.", Toast.LENGTH_SHORT).show();
        }
    }
        ...
        if (requestCode == 2296) {
            if (SDK_INT >= Build.VERSION_CODES.R) {
                if (Environment.isExternalStorageManager()) {
                    ...
                    Toast.makeText(DebtProfileActivity.this, "Allow permission for storage access!", Toast.LENGTH_SHORT).show();
                }
            }
        }
    }
