...
class ImageSelector : RecyclerView {
    ...
    private var allowDeleteOnEdit = false
    ...
    fun setPhotoList(data: ArrayList<AddedPhotoModel?>) {
        ...
        if (photoList.filterNotNull().size >= (formFields?.inputProps?.maxCount ?: 1)) {
            ...
            if (photoList.contains(null).not()){
                photoList.add(0,null)
            }
        }
    ...
    private fun openPhotoPicker(galleryPhotoDisabled : Boolean, model : AddedPhotoModel? = null, position : Int?= null) {
        ...
        if (model != null && position != null){
            ...
            val frag = ImageUploadBottomSheet(((formFields?.inputProps?.maxCount?:imageCount) - photoList.filterNotNull().size)) {
                if (it.isNotEmpty()) {
            ...
            fun bindItem(
                ...
                binding.imgDelete.setOnClickListener {
                    if (position < data.size) {
                        listener?.apply {
                            if (isOnEditMode && allowDeleteOnEdit.not()){
    ...
    fun setAllowDeleteOnEdit(allowDeleteOnEdit: Boolean) {
        this.allowDeleteOnEdit = allowDeleteOnEdit
    }
