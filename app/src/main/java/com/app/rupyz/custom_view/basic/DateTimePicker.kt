...
class DateTimePicker : LinearLayoutCompat {
    ...
    private var bardPastDaySelection : Boolean = false
    ...
    private fun setDefaultStyle() {
        ...
        inputLayout?.hint  = getLabel(true, formFields)
    ...
    private fun openDatePicker(timePickerEnabled: Boolean) {
        ...
        val constraint = if (bardPastDaySelection){
    ...
    fun getFieldValue(): NameAndValueSetInfoModel {
        ...
        model.value = if (inputLayout?.editText?.text.isNullOrBlank()) { null }else { DateFormatHelper.convertDateToIsoFormatFromDDMMYY(inputLayout?.editText?.text.toString()) }
        return model
    }
    ...
    fun setPastDatSelectionBard(bardPastDaySelection : Boolean){
        this.bardPastDaySelection = bardPastDaySelection
    }
