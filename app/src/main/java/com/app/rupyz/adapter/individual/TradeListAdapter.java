...
public class TradeListAdapter extends RecyclerView.Adapter<TradeListAdapter.ViewHolder> {
    ...
    public TradeListAdapter(List<Tradeline> listdata, Context mContext) {
...
/*        String sanctionAmount = mContext.getResources().getString(R.string.rs) + AmountHelper.getCommaSeptdAmount(AmountHelper.convertStringToDouble(
        ...
        notesBinding.btnDetails.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ((MyAccount) mContext).initBottomSheet(listdata.get(position));
            }
        });
        ...
        notesBinding.llAddEmi.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ((MyAccount) mContext).initAddEmiBottomSheet(listdata.get(position), position, 2);
            }
        });

        if (listdata.get(position).getScheduled_Monthly_Payment_Amount() !=null && listdata.get(position).getScheduled_Monthly_Payment_Amount() !=0){
        ...
        if (listdata.get(position).getRepayment_Tenure()!=null && listdata.get(position).getRepayment_Tenure()!=0){
            notesBinding.txtTenure.setText(listdata.get(position).getRepayment_Tenure()+" Month");
        }
        else {
            notesBinding.txtTenure.setText("-");
        }
        if (listdata.get(position).getRate_of_Interest()!=null  && listdata.get(position).getRate_of_Interest()!=0){
