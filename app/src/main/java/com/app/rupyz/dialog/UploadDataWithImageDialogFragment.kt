    ...
    fun setListener(actionModel: UploadingActionModel) {
        if (isAdded) {
            if (actionModel.imageExist == true && actionModel.imageCount != null) {
                ...
                binding.tvUploadingPicHeading.text = resources.getString(R.string.uploading_images_with_count,
            ...
            when (actionModel.type) {
                AppConstant.ATTENDANCE -> {
                    binding.tvUploadingTypeHeading.text = resources.getString(R.string.adding_attendance)
                    binding.ivAddProduct.setImageResource(R.drawable.ic_start_day_finger_print)
                }

                AppConstant.ACTIVITY -> {
                    binding.tvUploadingTypeHeading.text = resources.getString(R.string.adding_activity)
