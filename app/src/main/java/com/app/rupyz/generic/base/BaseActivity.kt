...
abstract class BaseActivity : AppCompatActivity(), NetworkChangeReceiver.NetworkChangeListener,
    ...
    open fun isStaffUserActivity(): Boolean {
        ...
        offlineViewModel.offlineDataAvailableLiveData.observe(this) { data ->
            if (data.first) {
                Handler(Looper.myLooper()!!).postDelayed({
                    startActivity(
                        Intent(this, UploadingOfflineDataActivity::class.java)
    ...
    fun addFragment(container: Int?, fragment: Fragment?) {
        //switching fragment
        if (fragment != null && supportFragmentManager.isDestroyed.not()) {
            supportFragmentManager
                .beginTransaction()
    ...
    fun logout() {
        ...
        logoutModel.deviceType = AppConstant.ANDROID

        val activityList = ArrayList<DeviceActivityListItem>()

        val model = DeviceActivityListItem()
        ...
        model.activityType = AppConstant.LOG_OUT
        activityList.add(model)

        logoutModel.data = activityList
        baseViewModel.logout(logoutModel)
