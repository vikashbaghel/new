...
public class DateFormatHelper {
    ...
    @SuppressLint("SimpleDateFormat")
    public static String convertDateToIsoFormatFromDDMMYY(String strDate) {
        DateFormat givenFormat = new SimpleDateFormat("dd MMM yyyy, hh:mm a" , Locale.ENGLISH);
        DateFormat originalFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss", Locale.ENGLISH);
        try {
            return originalFormat.format(Objects.requireNonNull(givenFormat.parse(strDate)));
        } catch (Exception exception) {
            try {
                return originalFormat.format(Objects.requireNonNull(originalFormat.parse(strDate)));
            } catch (Exception exe) {
    ...
    public static boolean isTimeGreaterThen30Second(@Nullable Long givenTimestamp) {
        ...
        long differenceInMillis = currentTimestamp - givenTimestamp;

        return (differenceInMillis > 30_000);
    }
}
