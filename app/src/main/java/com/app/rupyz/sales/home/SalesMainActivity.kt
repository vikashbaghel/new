...
import com.app.rupyz.sales.gallery.PictureGalleryActivity
...
class SalesMainActivity : BaseActivity(), OrganizationListAdapter.IOrgSelectListener,
    ...
    private fun initRecyclerView() {
        ...
        profileSelectedPosition = orgList.indexOfFirst { org -> org.isSelected == true }
    ...
    private fun initLayout() {
        ...
        binding.navigationLayout.llPictureGallery.setOnClickListener {
            closeDrawer()
            Handler(Looper.myLooper()!!).postDelayed({
                startActivity(Intent(this, PictureGalleryActivity::class.java))
               // SharedPref.getInstance().getArrayList(AppConstant.St)
            }, 300)
        }
    ...
    private fun initData() {
        if (SharedPref.getInstance().getInt(ORG_ID) == 0) {
            ...
            if (orgModel.legalName.isNullOrEmpty().not()) {
