...
class CustomerRepository {
    ...
    fun getCustomerList(
        ...
        customerFilterParams: HashMap<String, String> = hashMapOf()
    ) {
        ...
        val uploadCred: Call<CustomerInfoModel> =
            if (pageSize == null) {
                RetrofitClient.apiInterface.getCustomerList(
                    ...
                    page,
                    null,
                    null, null, customerFilterParams
                )
            } else {
    ...
    fun getCustomerListMapped(
        ...
        val uploadCred: Call<CustomerInfoModel> =
            if (pageSize == null) {
                RetrofitClient.apiInterface.getCustomerListMapped(
                    org_id = id,
                    customer_id = customerId,
                    name = name,
                    customer_level = customerLevel,
                    customer_type = stringBuilder,
                    sortBy = sortBy,
                    sortByOrder = sortingOrder,
                    page_no = headers.divideHeadersIntoQueryParams().second,
                    pageSize = 30,
                    forDump = null,
                    selected = headers.divideHeadersIntoQueryParams().first,
                    offlineDataLastSyncedTime = null,
                    ignoreMapping = ignoreMapping
                )
            } else {
                OfflineRetrofitClient.offlineApiInterface.getCustomerListMapped(
                    org_id = id,
                    customer_id = customerId,
                    name = name,
                    customer_level = customerLevel,
                    ...
                    forDump = true,
                    selected = headers.divideHeadersIntoQueryParams().first,
                    offlineDataLastSyncedTime = offlineDataLastSyncedTime,
                    ignoreMapping = ignoreMapping
                )
            }
    ...
    fun getCustomerByIdNew(liveData: MutableLiveData<NewUpdateCustomerInfoModel>, customerId: Int) {
        val uploadCred: Call<NewUpdateCustomerInfoModel> =
            RetrofitClient.apiInterface.getCustomerByIdNew(
                SharedPref.getInstance().getInt(ORG_ID),
                customerId
            )

        uploadCred.enqueue(object : NetworkCallback<NewUpdateCustomerInfoModel?>() {
            override fun onSuccess(t: NewUpdateCustomerInfoModel?) {
                CoroutineScope(Dispatchers.IO).launch {
                    liveData.postValue(t)
                }
            }
            ...
            override fun onFailure(failureResponse: FailureResponse?) {
                val staffAddResponseModel = NewUpdateCustomerInfoModel()
                try {
                    val parser = JsonParser()
                    val jsonObj: JsonObject? =
                        parser.parse(failureResponse?.errorBody) as JsonObject?
    ...
    fun getOrderTakenByAdmin(
        liveData: MutableLiveData<OrderTakenByAdminModel>,
        pageNo: Int,
        name: String,
        dd: Boolean
    ) {
        val uploadCred: Call<OrderTakenByAdminModel> =
            RetrofitClient.apiInterface.getOrderTakenByAdmin(
                SharedPref.getInstance().getInt(ORG_ID),
                pageNo, name, dd
            )

        uploadCred.enqueue(object : NetworkCallback<OrderTakenByAdminModel?>() {
            override fun onSuccess(t: OrderTakenByAdminModel?) {
                CoroutineScope(Dispatchers.IO).launch {
                    liveData.postValue(t)
                }
            }
            ...
            override fun onFailure(failureResponse: FailureResponse?) {
                val orderTakenByAdminModel = OrderTakenByAdminModel()
                try {
                    val parser = JsonParser()
                    val jsonObj: JsonObject? =
                        parser.parse(failureResponse?.errorBody) as JsonObject?

                    orderTakenByAdminModel.error = true
                    orderTakenByAdminModel.errorCode = failureResponse?.errorCode
                    orderTakenByAdminModel.message = jsonObj?.get("message")?.asString

                } catch (e: Exception) {
                    e.printStackTrace()
                }
                CoroutineScope(Dispatchers.IO).launch {
                    liveData.postValue(orderTakenByAdminModel)
                }
            }
    ...
    fun getCheckIn(liveData: MutableLiveData<CheckInResponse>, checkInRequest: CheckInRequest) {
        val uploadCred: Call<CheckInResponse> =
            RetrofitClient.apiInterface.getCheckIn(
                SharedPref.getInstance().getInt(ORG_ID),
                checkInRequest
            )

        uploadCred.enqueue(object : NetworkCallback<CheckInResponse?>() {
            override fun onSuccess(t: CheckInResponse?) {
                CoroutineScope(Dispatchers.IO).launch {
                    liveData.postValue(t)
                }
            }
            ...
            override fun onFailure(failureResponse: FailureResponse?) {
                ...
                try {
                    ...
                    checkIn.error = true
                    checkIn.errorCode = failureResponse?.errorCode
                    checkIn.message = jsonObj?.get("message")?.asString

                } catch (e: Exception) {
                    e.printStackTrace()
                }
                CoroutineScope(Dispatchers.IO).launch {
                    liveData.postValue(checkIn)
                }
            }
    ...
    fun getCheckOut(liveData: MutableLiveData<CheckInResponse>, checkoutRequest: CheckoutRequest) {
        val uploadCred: Call<CheckInResponse> =
            RetrofitClient.apiInterface.getCheckOut(
                SharedPref.getInstance().getInt(ORG_ID),
                checkoutRequest
            )
    ...
    fun addCustomerNew(
        ...
        jsonData: AddNewCustomerFormDataModel
    ) {
        val uploadCred: Call<CustomerAddResponseModel> =
            RetrofitClient.apiInterface.addCustomerNew(
                jsonData,
                SharedPref.getInstance().getInt(ORG_ID)
            )
    ...
    fun updateCustomerNew(
        liveData: MutableLiveData<NewUpdateCustomerInfoModel>,
        jsonData: AddNewCustomerFormDataModel,
        customerId: Int
    ) {
        val uploadCred: Call<NewUpdateCustomerInfoModel> =
            RetrofitClient.apiInterface.updateCustomerNew(
                jsonData,
                SharedPref.getInstance().getInt(ORG_ID),
                customerId
            )
    ...
    fun getStaffListWithCustomerMapping(
        ...
        val uploadCred: Call<StaffListWithCustomerMappingModel> =
            RetrofitClient.apiInterface.getStaffListUsingCustomerMapping(
                id, customerId, name,
    ...
    fun getAddNewCustomerFormData(
        liveData: MutableLiveData<CustomFormCreationModel>,
        pageNo: Int,
        isOnline: Boolean
    ) {
        val id = SharedPref.getInstance().getInt(ORG_ID)
        val uploadCred: Call<CustomFormCreationModel> = if (isOnline) {
            RetrofitClient.apiInterface.getAddCustomerCustomFormData(id, pageNo)
        } else {
            OfflineRetrofitClient.offlineApiInterface.getAddCustomerCustomFormData(id, pageNo)
        }
        uploadCred.enqueue(object : NetworkCallback<CustomFormCreationModel?>() {
            ...
            override fun onFailure(failureResponse: FailureResponse?) {
                val model = CustomFormCreationModel()
                try {
                    val parser = JsonParser()
                    var jsonObj: JsonObject? = null
                    jsonObj = parser.parse(failureResponse?.errorBody) as JsonObject?
...
//        CoroutineScope(Dispatchers.IO).launch {
//            val data  = TempData.getCustomAddNewCustomerData(pageNo)
//            val formData  = Gson().fromJson<CustomFormCreationModel>(data,CustomFormCreationModel::class.java)
//            liveData.postValue(formData)
//        }

    }

    fun getUserCheckInStatus(liveData: MutableLiveData<CheckInStatus>, isOnline: Boolean) {
        val id = SharedPref.getInstance().getInt(ORG_ID)
        if (isOnline) {
            val uploadCred: Call<CheckInStatus> =
                RetrofitClient.apiInterface.getUserCheckInStatus(id)
            uploadCred.enqueue(object : NetworkCallback<CheckInStatus?>() {
                override fun onSuccess(t: CheckInStatus?) {
                    CoroutineScope(Dispatchers.IO).launch {
                        liveData.postValue(t)
                    }
                }
            ...
            try {
                model.error = true
                model.message = AppConstant.NO_INTERNET_CONNECTION
