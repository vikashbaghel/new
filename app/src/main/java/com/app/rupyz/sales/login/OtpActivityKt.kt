...
class OtpActivityKt : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        updatedAt = intent.getLongExtra(AppConstant.UPDATE_AT, 0)
        ...
        smsBroadcastReceiver?.smsBroadcastReceiverListener =
            object : SmsBroadcastReceiver.SmsBroadcastReceiverListener {
                ...
                override fun onFailure() {
                    log("Failed:: smsBroadcastReceiverListener")
                }
            }
    ...
    private fun initTimer() {
        ...
        // Total countdown duration in milliseconds (e.g., 30 seconds)
        val totalDuration: Long = 30_000

        var elapsedTime = 0L
        // Calculate elapsed time
        if (updatedAt != 0L) {
            elapsedTime = currentTimestamp - updatedAt
        }
    ...
    private fun initObservers() {
        moreViewModel.preferenceLiveData.observe(this) {
            if (it.error == false) {
                ...
                SharedPref.getInstance().putString(AppConstant.INITIATE_LOGIN, null)
            ...
            if (it.error == false) {
                it.data?.let { mUserData ->
                    ...
                    if (isStaffUser) {
                        ...
                        deviceModel.activityType = AppConstant.LOG_IN
                        deviceModel.activityTimeStamp =
                            DateFormatHelper.convertDateToIsoUTCFormat(Calendar.getInstance().time)
                        deviceModel.deviceInformation = getDeviceInformation()
                        deviceModel.batteryPercent = getBatteryInformation().first
                        deviceModel.isSystemPowerSaving = isBatteryOptimizationEnabled()
                        deviceModel.isAppPowerSaving = isMyAppIsBatteryOptimizationMode()
                        deviceModel.locationPermission = isGpsEnabled()
                ...
                if (it.message?.contains("Invalid", true) == true && it.message?.contains(
                        "Request",
                        true
                    ) == true
                ) {
                    showToast(resources.getString(R.string.invalid_otp))
                } else {
                    showToast(it.message ?: resources.getString(R.string.something_went_wrong))
                }
            }
