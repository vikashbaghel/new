...
class ChooseActivityBottomSheet : BottomSheetDialogFragment(),
    ...
    override fun onCreate(savedInstanceState : Bundle?) {
        ...
        activityResultContract = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result: ActivityResult ->
            if (result.resultCode == RESULT_OK) {
                ...
                CheckOutConfiramation.showCheckOutConfirmationDialog(requireActivity(), customerName = customerData?.name ?: "", orderMessage, customerData?.id?: 0, object : ICheckOutConfirmationClickListener{
    ...
    private fun setListener() {
        ...
        activityListAdapter.setActivitySelectListener { activity ->
            when (activity.id) {
                /***
                ...
                999989 -> {
                    customerData?.let { customerData ->
                        if (PermissionModel.INSTANCE.getPermission(AppConstant.VIEW_PAYMENT_PERMISSION, false)) {
                            activityResultContract?.launch(Intent(requireActivity(), AddRecordPaymentActivity::class.java)
                                            .putExtra(AppConstant.CUSTOMER, customerData)
                            ...
                            if (requireActivity() is BaseActivity) {
                                (requireActivity() as BaseActivity).showToast(resources.getString(R.string.payment_permission))
                            }else{
                                Toast.makeText(requireActivity(), resources.getString(R.string.payment_permission), Toast.LENGTH_SHORT).show()
                            }
                        }
                    }
                }
                    ...
                    customerData?.let { customerData ->
                        activityResultContract?.launch(
                            Intent(context, CustomFormActivity::class.java)
                                .putExtra(AppConstant.CUSTOMER, customerData)
                                .putExtra(AppConstant.CUSTOMER_ID, customerData.id ?: 0)
                                .putExtra(AppConstant.FEEDBACK_ID, activity.id)
                                .putExtra(AppConstant.ACTIVITY_TYPE, moduleType)
                        )
                    }
                }
    ...
    private fun initObservers() {
        checkOutViewModel.getCheckOut().observe(this) { data ->
            if (data.error == false) {
                if (requireActivity() is BaseActivity) {
                    (requireActivity() as BaseActivity).showToast(data.message)
                }
                val intent = Intent(requireActivity(), ListOfCustomerActivity::class.java)
        ...
        activityViewModel.getFollowUpListLiveData.observe(this) {
            if (it.error == false) {
                ...
                if (activity is BaseActivity) {
                    (activity as BaseActivity).showToast(it.message)
                }
            }
        }
    }
   ...
   /* override fun onConfirm(customerName: String, customerID: Int) {
    ...
    override fun onCheckoutConfirm(customerName: String, customerID: Int) {
        ...
        checkOutViewModel.getCheckOutData(checkoutRequest, hasInternetConnection(requireContext()))
    }
