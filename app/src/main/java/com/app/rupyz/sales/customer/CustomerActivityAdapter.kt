...
//            }
            ...
            binding.tvSubModuleType.text = model.createdByName
            try{
                binding.ivUserProfileImage.setImageBitmap(StringHelper.printName(model.createdByName).trim().substring(0, (Math.min(model.createdByName?.length?:0, 2))).uppercase().asBitmap(binding.root.context, 16f, Color.WHITE, binding.root.context.resources.getColor(R.color.theme_color, null)))
            }catch (e: Exception){
                log(e.toString())
            }
