...
class ImageUploadHandler : FormItemHandler, MultipleImageUploadListener,
    ...
    private var oldPicsUrls: ArrayList<PicMapModel> = arrayListOf()
    ...
    override fun onDeleteImage(
        ...
        val oldRemovedValue = oldPicsUrls.firstOrNull { it.url?.equals(photoModelList[position]?.imagePath) == true }
        if ( oldRemovedValue != null){
            val model = formItemModels?.find { it.name == formItem?.fieldProps?.name }
            model?.imgUrls?.removeAll { it.trim().equals(oldRemovedValue.url,true)   }
            model?.imgUrls?.removeIf { it.trim().equals(oldRemovedValue.url,true)   }
        }
