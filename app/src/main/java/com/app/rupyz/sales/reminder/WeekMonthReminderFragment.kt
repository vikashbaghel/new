...
class WeekMonthReminderFragment : BaseFragment(),
    DateWiseReminderListAdapter.IReminderListDetailsListener,
    ...
    private lateinit var adapter: DateWiseReminderListAdapter

    private var reminderList = ArrayList<SortReminderListByDate>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (arguments?.getString(AppConstant.TAB_NAME) != null) {
            category = arguments?.getString(AppConstant.TAB_NAME)!!
        } else if (arguments?.getString(AppConstant.DATE_FILTER) != null) {
            category = AppConstant.DATE
            particularDate = arguments?.getString(AppConstant.DATE_FILTER)!!
        }
    ...
    private fun separateListForDate(list: ArrayList<ReminderItemModel>) {
        val groupedMapMap: Map<String, List<ReminderItemModel>> =
        ...
        for (date: String in groupedMapMap.keys) {
            reminderList.add(DateItem(date))
            val groupItems: List<ReminderItemModel>? = groupedMapMap[date]
            groupItems?.forEach {
                reminderList.add(it)
            }
        }
    ...
    override fun onGetReminderDetails(model: ReminderItemModel) {
        if (model.moduleId != -1) {
            ...
            fragment.setOnItemUpdatedListener {
                currentPage = 1
                loadReminderList()
            }
        } else {
            showToast(getString(R.string.customer_not_found))
        }
    }
