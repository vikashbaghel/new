...
class StartDayEndDayDetailsActivity : BaseActivity(), ProductImageViewPagerAdapter.ProductImageClickListener {
    ...
    private fun initData() {
        ...
        if (intent.hasExtra(AppConstant.ATTENDANCE)) {
            attendanceModel = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                intent.getParcelableExtra(AppConstant.ATTENDANCE, CustomerFollowUpDataItem::class.java)
            } else {
            ...
            if (attendanceModel?.geoLocationLat != 0.0 && attendanceModel?.geoLocationLong != 0.0) {
                ...
                if (attendanceModel?.geoAddress.isNullOrBlank().not()) {
                    ...
                    binding.tvLocations.text = GeoLocationUtils.getAddress(this,
                            attendanceModel?.geoLocationLat!!, attendanceModel?.geoLocationLong!!)
                }

                binding.tvLocations.setOnClickListener {
                    Utils.openMap(this, attendanceModel?.geoLocationLat!!,
                    ...
                    binding.apply {
                        ...
                        when (data.activityType) {
                            ...
                            ACTIVITY_TYPE_DISTRIBUTOR_VISIT -> {
                                tvDayStatus.text = resources.getString(R.string.distributor_visit,
                        ...
                        if (data.isAutoTimeOut != null && data.isAutoTimeOut == true) {
