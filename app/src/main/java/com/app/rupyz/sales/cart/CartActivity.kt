...
class CartActivity : BaseActivity(), OnCartActionListener, CartDiscountDeleteListener,
    ...
    private fun submitData() {
        ...
        if (orderImageLisForUpload.size > 0) {
...
//            cartListResponseModel?.orderId = orderData?.orderId
//            cartListResponseModel?.totalAmount = orderData?.totalAmount
        }

        orderViewModel.updateOrder(cartListResponseModel?.id!!, cartListResponseModel, hasInternetConnection())
    }
            ...
            if (result.data?.hasExtra(AppConstant.CART_ITEM)!!) {
                ...
                if (refactorCartModel?.paymentDetails?.amount != null) {
                    binding.tvPaymentAmount.text =
                            CalculatorHelper().convertLargeAmount(refactorCartModel?.paymentDetails?.amount!!,
                ...
                if (refactorCartModel?.paymentOptionCheck == AppConstant.CREDIT_DAYS_API) {
                    ...
                    when (refactorCartModel?.paymentOptionCheck) {
                        AppConstant.FULL_PAYMENT_IN_ADVANCE_API ->
                            binding.tvPaymentTerms.text = "Payment Terms : ${
                ...
                cartListResponseModel?.remainingPaymentDays = refactorCartModel?.remainingPaymentDays
        ...
        cartItemList.forEach {
            ...
            orderAmount = orderAmount.plus(
                    CalculatorHelper().calculatePriceWithoutGst(
                            ...
                            it.gst!!,
                            it.gst_exclusive!!, AppConstant.FOUR_DECIMAL_POINTS
                    )
            )
    ...
    private fun manageDiscount() {
        ...
        if (cartItemDiscountList.size > 0) {
            ...
            binding.tvDiscountAmount.text =
                    CalculatorHelper().convertCommaSeparatedAmount(totalDiscountAmount, AppConstant.TWO_DECIMAL_POINTS)
    ...
    private fun manageTotalAmount() {
        ...
        binding.tvOrderTotalAmount.text =
                CalculatorHelper().convertCommaSeparatedAmount(totalOrderAmountWithoutGst, AppConstant.TWO_DECIMAL_POINTS)
        binding.tvOrderTotalGst.text =
                CalculatorHelper().convertCommaSeparatedAmount(totalGstAmountForFinalCalculation, AppConstant.TWO_DECIMAL_POINTS)

        totalCartAmount =
                totalOrderAmountWithoutGst.plus(totalDeliverCharges)
                        .plus(totalGstAmountForFinalCalculation)

        cartListResponseModel?.amount = CalculatorHelper().roundDecimalPoint(totalCartAmount, AppConstant.TWO_DECIMAL_POINTS)
        cartListResponseModel?.paymentAmount = CalculatorHelper().roundDecimalPoint(totalCartAmount, AppConstant.TWO_DECIMAL_POINTS)

        binding.tvOrderTotalAmountInc.text =
                CalculatorHelper().convertCommaSeparatedAmount(totalCartAmount, AppConstant.TWO_DECIMAL_POINTS)
    ...
    private fun splitAmountByRatio(
        ...
        cartItemList.forEach {
            ...
            discountedTotalGstAmountSingleItem =
                    amountWithoutGst - discountRatioAmountSingleItem
    ...
    private fun initSharedPrefObservers() {
        if (SharedPref.getInstance().getString(CART_MODEL).isNullOrEmpty().not()) {
            ...
            if (!response.equals("")) {
                ...
                if (cartListResponseModel?.customerId == customerId) {
                    ...
                    binding.tvCompanyName.text = cartListResponseModel?.customerName
        ...
        if (cartListResponseModel?.address?.id != null) {
            ...
            binding.tvAddressLine.text =
                    "${cartListResponseModel?.address?.city}," +
                            " ${cartListResponseModel?.address?.state}," +
                            " ${cartListResponseModel?.address?.pincode}"
        ...
        if (cartListResponseModel?.paymentDetails != null) {
            cartListResponseModel?.paymentDetails?.let {
                ...
                if (it.amount != null) {
                    binding.tvPaymentAmount.text =
                            CalculatorHelper().convertLargeAmount(it.amount!!, AppConstant.TWO_DECIMAL_POINTS)
                    binding.tvPaymentAmount.visibility = View.VISIBLE
                } else {
                    binding.tvPaymentAmount.visibility = View.GONE
                }
        ...
        if (!cartListResponseModel?.discountDetails.isNullOrEmpty()) {
            cartItemDiscountList.clear()
            cartItemDiscountList.addAll(cartListResponseModel?.discountDetails!!)
            cartItemsDiscountListAdapter.notifyDataSetChanged()
            if (cartItemDiscountList.size > 0) {
        ...
        if (cartListResponseModel?.chargesDetails != null && cartListResponseModel?.chargesDetails.isNullOrEmpty().not()) {
            cartItemOthersChargesList.clear()
            cartItemOthersChargesList.addAll(cartListResponseModel?.chargesDetails!!)
            cartItemsOthersChargesAdapter.notifyDataSetChanged()
            if (cartItemOthersChargesList.size > 0) {
            ...
            totalDeliverCharges = 0.0
            manageOthersCharges()
        } else if (cartListResponseModel?.deliveryCharges != null && cartListResponseModel?.deliveryCharges!!.roundToInt() != 0) {
            binding.tvAddDeliveryCharges.visibility = View.GONE
        ...
        binding.ivEditPaymentAgainstOrder.setOnClickListener {
            paymentAgainstOrderResultLauncher.launch(
                    Intent(
                            this,
                            PaymentAgainstOrderActivity::class.java
                    ).putExtra(AppConstant.CART_ITEM, cartListResponseModel)
    ...
    private fun initApiObservers() {
        ...
        imageUploadViewModel.getCredLiveData().observe(this) { genericResponseModel ->
            if (genericResponseModel.error == false) {
                ...
                Toast.makeText(this, "${genericResponseModel.message}", Toast.LENGTH_SHORT)
                        .show()
                progressDialog.dismiss()
            }
        }
    ...
    private fun initProductData(
        ...
        binding.tvAddProduct.visibility = View.VISIBLE

//        cartListResponseModel = OrderData()
//        cartListResponseModel.customerId = customerId
//        cartListResponseModel.customerName = orderData?.customer?.name
//        cartListResponseModel.amount = orderData?.totalAmount
//        cartListResponseModel.paymentAmount = orderData?.totalAmount
//        cartListResponseModel.comment = orderData?.comment
//        cartListResponseModel.adminComment = orderData?.adminComment
//        cartListResponseModel.paymentOptionCheck = orderData?.paymentOptionCheck
//        cartListResponseModel.remainingPaymentDays = orderData?.remainingPaymentDays
//        cartListResponseModel.paymentDetails = orderData?.paymentDetails
//        cartListResponseModel.paymentMode = orderData?.paymentDetails?.paymentMode
//        cartListResponseModel.items = orderData?.items
//        cartListResponseModel.chargesDetails = orderData?.chargesDetails
//        cartListResponseModel.deliveryCharges = orderData?.deliveryCharges
//        cartListResponseModel.orderImagesInfo = orderData?.orderImagesInfo
//
//        SharedPref.getInstance().putModelClass(CART_MODEL, cartListResponseModel)
