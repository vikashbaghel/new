...
class LeadDetailsActivity : BaseActivity(),
    ...
    private val moreViewModel: MoreViewModel by viewModels<MoreViewModel>()
    ...
    private fun initLayout() {
        ...
        binding.btnConvertToCustomer.setOnClickListener {
            if (leadModel.status == AppConstant.STATUS_APPROVED) {
                if (leadModel.mobile != null){
                    binding.progressBar.showView()
                    moreViewModel.validateWhatsAppNumber(leadModel.mobile!!, AppConstant.MODULE_CUSTOMER)
                }else{
                    someActivityResultLauncher.launch(
                        Intent(
                            this,
    ...
    private fun initObservers() {
        ...
        moreViewModel.verifyWhatsAppNumberLiveData.observe(this) { data ->
            binding.progressBar.hideView()
            data?.let {
                if (data.data?.isUsed == true){
                    data.message?.let { it1 -> numberAlreadyUsed(it1) }
                }else{
                    someActivityResultLauncher.launch(
                        Intent(
                            this,
    ...
    private fun numberAlreadyUsed(message: String) {
        val fragment = InfoBottomSheetDialogFragment()
        val bundle = Bundle()
        bundle.putString(AppConstant.HEADING,resources.getString(R.string.number_already_used))
        bundle.putString(
            AppConstant.MESSAGE,
           message
        )
