        ...
        uploadCred.enqueue(object : NetworkCallback<UserLoginResponseModel?>() {
            override fun onSuccess(t: UserLoginResponseModel?) {
                CoroutineScope(Dispatchers.IO).launch {
                    // Your Triple
                    t?.data.let { user ->
                        val triple = Triple(loginModel.username, System.currentTimeMillis(), user?.otpRef)
                        // Convert Triple to a string (you can use any separator, here ":" is used)
                        val tripleString = "${triple.first}:${triple.second}:${triple.third}"
                        SharedPref.getInstance().putString(AppConstant.INITIATE_LOGIN, tripleString)
                    }
