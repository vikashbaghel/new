package com.app.rupyz.sales.targets

import android.annotation.SuppressLint
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import androidx.recyclerview.widget.RecyclerView
import com.app.rupyz.R
import com.app.rupyz.databinding.ViewPagerItemTargetListBinding
import com.app.rupyz.generic.utils.AppConstant
import com.app.rupyz.generic.utils.CalculatorHelper
import com.app.rupyz.model_kt.StaffTargetModel

class TargetsListViewPagerAdapter(
    private var data: ArrayList<StaffTargetModel>,
    private var listener: ITargetActionListener
) :
    RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val itemView = LayoutInflater.from(parent.context)
            .inflate(R.layout.view_pager_item_target_list, parent, false)
        return MyViewHolder(itemView)
    }

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        (holder as MyViewHolder).bindItem(data[position], position, listener)
    }

    override fun getItemCount(): Int {
        return data.size
    }

    class MyViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        private val binding = ViewPagerItemTargetListBinding.bind(itemView)
        @SuppressLint("SetTextI18n")
        fun bindItem(model: StaffTargetModel, position: Int, listener: ITargetActionListener) {
            binding.tvHeading.text = model.title

            if (model.title == AppConstant.TARGET_SALES || model.title == AppConstant.TARGET_COLLECTION) {
                var targetAmount = CalculatorHelper().convertCommaSeparatedAmount(model.target, AppConstant.TWO_DECIMAL_POINTS)
                if (targetAmount!!.contains(".00")) {
                    targetAmount = targetAmount.replace(".00", "")
                }
                binding.tvTarget.text = targetAmount

                var achievedAmount = CalculatorHelper().convertCommaSeparatedAmount(model.achieved, AppConstant.TWO_DECIMAL_POINTS)
                if (achievedAmount!!.contains(".00")) {
                    achievedAmount = achievedAmount.replace(".00", "")
                }

                binding.tvAchieved.text = achievedAmount

            } else {
                binding.tvTarget.text = "${model.target?.toInt()}"
                binding.tvAchieved.text = "${model.achieved?.toInt()}"

                if (model.title == AppConstant.TARGET_PRODUCTS) {
                    binding.groupAchieve.visibility = View.GONE
                    binding.tvProductDetails.visibility = View.VISIBLE

                    binding.hdTarget.text = itemView.resources.getString(R.string.number_of_products)
                } else {
                    binding.groupAchieve.visibility = View.VISIBLE
                    binding.tvProductDetails.visibility = View.GONE

                    binding.hdTarget.text = itemView.resources.getString(R.string.targets)
                }
            }

            binding.ivTargetLogo.setImageResource(model.logo!!)

            if (model.title == AppConstant.TARGET_PRODUCTS) {
                binding.progressBar.progress = model.achieved!!.toInt()
                binding.progressBar.title = "${model.achieved!!.toInt()}%"

                itemView.setOnClickListener {
                    listener.getTargetProductDetails()
                }

            } else{
                if (model.achieved != 0.0) {
                    val progress = (model.achieved!! / model.target!!) * 100
                    binding.progressBar.progress = progress.toInt()
                    binding.progressBar.title = "${progress.toInt()}%"
                } else {
                    binding.progressBar.progress = 0
                    binding.progressBar.title = "0%"
                }

                itemView.setOnClickListener {
                    listener.getTargetDetails()
                }
            }


        }
    }

    interface ITargetActionListener {
        fun getTargetDetails()
        fun getTargetProductDetails()
    }
}