...
class MyActivityFragment : BaseFragment(),
    ...
    private var liveLocationPoints: ArrayList<CustomerFollowUpDataItem> = ArrayList()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            ...
            if (arguments?.getString(AppConstant.STAFF_NAME) != null) {
                staffName = arguments?.getString(AppConstant.STAFF_NAME) ?: ""
            }
        }
...
//        }
        ...
        binding.clTrackingInfo.clDistanceTravelled.setOnClickListener {
            ...
            bundle.putString(
                AppConstant.HEADING,
                resources.getString(R.string.distance_calculation)
            )
            bundle.putString(
                AppConstant.MESSAGE,
                resources.getString(R.string.distance_calculation_message)
            )
                ...
                if (it.data.isNullOrEmpty().not()) {
                    ...
                    it.data?.forEach { dataModel ->
                        ...
                        if (dataModel.latitude != 0.0 && dataModel.longitude != 0.0) {
                            liveLocationPoints.add(
                                CustomerFollowUpDataItem(geoLocationLat = dataModel.latitude,
                                    geoLocationLong = dataModel.longitude, createdAt = dataModel.dateTime))

//                            if(liveLocationPoints.size == 1){
//                                liveLocationPoints[0].moduleType = AppConstant.ATTENDANCE
//                                liveLocationPoints[0].action = AppConstant.ATTENDANCE_CHECK_IN
//                            }
                        }
                    }
...
//        val activityMapPointsModel = ActivityMapPointsModel(null, mapPointsList)
        startActivity(
                Intent(requireContext(), GoogleMapActivity::class.java)
            ...
            AppConstant.DAY_START_END -> {
                ...
                startActivity(
                        Intent(
                            ...
                            .putExtra(AppConstant.ACTIVITY_ID, model.id)
                           // .putExtra(AppConstant.CUSTOMER_ID, staffId )
                            .putExtra(AppConstant.CUSTOMER_ID, model.moduleId)
